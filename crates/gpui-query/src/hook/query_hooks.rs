...
// Vendored fix (not in any published release): only the release-profile
// fallback below calls `AppContext::new`, so the import is cfg-gated to
// match — unconditional it warns as unused in dev builds.
